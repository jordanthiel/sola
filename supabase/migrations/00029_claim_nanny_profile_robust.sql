-- Robust claim: idempotent re-open, active nanny membership, avoid pto unique conflicts
...
BEGIN
  ...
  -- Already linked via household_nannies row
  SELECT * INTO v_nanny
  ...
  -- Already linked via nanny membership (e.g. profile row not updated)
  ...
  -- Re-open invite: token still on URL but row already claimed by this user
  ...
  -- Match by email for already-claimed profile (token cleared after first claim)
  SELECT * INTO v_nanny
  FROM household_nannies
  ...
  -- Only touch rows still unassigned to avoid unique (household_id, nanny_user_id) conflicts
